...
void AMushActor::BeginPlay()
{
	...
	if (topBoxCollider)
		topBoxCollider->OnComponentBeginOverlap.AddDynamic(this, &AMushActor::OnTopBoxBeginOverlap);
...
void AMushActor::OnTopBoxBeginOverlap(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult) {
	if (topBoxCollider)
		topBoxCollider->OnComponentBeginOverlap.RemoveAll(this);
	Destroy();
}
